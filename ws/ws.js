...
const system_ws = require('./system_ws');
...
module.exports = (io) => {
    io.on('connection', async (socket) => {
        ...
        socket.on('data', async (result) => {
            ...
            } else if (result.code === config.wsCode.CREATE_SYSTEM) {
                // create system
                system_ws.createSystem(
                    io,
                    socket_users,
                    payload.name,
                    payload.intro,
            ...
            } else if (result.code === config.wsCode.FETCH_SYSTEMS) {
                system_ws.getSystems(socket);
            } else if (result.code === config.wsCode.FETCH_SYSTEMS_FILE) {
                system_ws.getSystemFiles(socket, payload.systemId, userId);
            } else if (result.code === config.wsCode.CREATE_SYSTEM_MESSAGE) {
                system_ws.updateSystemFiles(io, socket_users, payload.systemId);
            }
        });
    });
};
