...
function shouldUpdate(data) {
    ...
    } else if (data.code === 928) {
        renderSystems(data);
    } else if (data.code === 931) {
        renderSystemFiles(data);
    }
    $('.spinner').hide();
}

function renderSystemFiles(data) {
    let system = data.detail.system;
    let files = data.detail.files;
    let isAdmin = data.detail.isAdmin;
    $('#system_detail_div').html(`
    <table class="table table-bordered task-detail-table">
      <tbody>
        <tr>
          <th>系统名</th>
          <td colspan=3>${system.name}</td>
        </tr>
        <tr>
          <td colspan=4>
            <h4>系统简介:</h4>${system.intro ? system.intro : '无'}
          </td>
        </tr>
        <tr>
          <th>文档数</th>
          <td>${system.docCount}</td>
          <th>创建时间</th>
          <td>${system.createdAt ? moment(system.createdAt).format('YYYY-MM-DD HH:mm:ss') : '无'}</td>
        </tr>
      </tbody>
    </table>
  `);
    // 渲染附件
    let attachmentLis = _.map(files, (item) => {
        let isImage = item.ext === 'jpg' || item.ext === 'jpeg' || item.ext === 'png' || item.ext === 'gif';
        return `
        <li>
        <p>
        <a href="${item.url}" ${isImage ? 'target="_blank"' : 'download'}>${item.name}</a>
        ${ isAdmin === 1 ? `<button type="button" class="btn btn-primary btn-xs pull-right" onclick="deleteSystemAttachments('${item.url}',${system.id})">删除</button>` : ""}
        </p>
        </li>
      `;
    }).join('');
    ...
    $('#system_attachment_div').html(`
    <p><span style="font-size: 18px;">附件</span> ${ isAdmin === 1 ? "<button type=\"button\" class=\"btn btn-primary btn-xs pull-right\" onclick=\"toUploadFile()\">上传附件</button>" : ""}</p>
    <input type="file" class="hidden" id="uploadFileInput" multiple onchange="uploadSystemFile(${system.id})"/>
    <ul style="margin-bottom: 20px;">${attachmentLis}</ul>
  `);
}

function uploadSystemFile(id) {
    let files = document.getElementById('uploadFileInput').files;
    let fd = new FormData();
    fd.append('systemId', id);
    for (let i = 0; i < files.length; i++) {
        fd.append(`files`, files[i]);
    }
    $.post({
        url: '/uploadSystemFile',
        data: fd,
        dataType: 'json',
        processData: false,
        contentType: false,
...
function renderSystems(data) {
    let divs = _.map(data.detail.systems, (system) => {
        return `
      <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3">
        <a data-pjax href="/system/${system.id}">
          <div class="well well-sm" style="background-color: #fff;">
            ...
            <p>
              系统简介：${system.intro ? system.intro : '无'}&nbsp;
            </p>
          </div>
        </a>
      </div>`;
...
function renderTask(data) {
  ...
  `);
    ...
    let attachmentLis = _.map(attachments, (item) => {
        ...
        <p>
        <a href="${item.url}" ${isImage ? 'target="_blank"' : 'download'}>${item.name}</a>
        <button type="button" class="btn btn-primary btn-xs pull-right" onclick="deleteAttachments('${item.url}',${task.id})">删除</button>
        </p>
        </li>
      `;
    }).join('');
...
function deleteAttachments(url, taskid) {
    $.post('/deleteFile', {
        url: url
    }, function (data) {
        if (data.code === 200) {
            ...
            ws.emit('data', {
                ...
                detail: {
                    taskId: taskid,
                    content: `${sessionStorage.getItem('username')}删除了1个附件，文件路径为` + url,
                    mentionUserIds: []
                }
            });
        }
...
function deleteSystemAttachments(url, systemid) {
    $.post('/deleteFile', {
        url: url,
        type: 'system',
        systemid: systemid
